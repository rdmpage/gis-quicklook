...
#define PLUGIN_ID "2422AB72-8D38-4055-8F8A-29218D8C73E1"
...
HRESULT QuickLookGeneratorQueryInterface(void *thisInstance,REFIID iid,LPVOID *ppv)
{
    ...
    bool match = CFEqual(interfaceID, kQLGeneratorCallbacksInterfaceID);
    os_log(OS_LOG_DEFAULT, "GISLook: QueryInterface called, match=%d", match);
...
void *QuickLookGeneratorPluginFactory(CFAllocatorRef allocator,CFUUIDRef typeID)
{
    ...
    os_log(OS_LOG_DEFAULT, "GISLook: factory called, typeMatch=%d", CFEqual(typeID,kQLGeneratorTypeID));
